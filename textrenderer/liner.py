...
class LineState(object):
    tableline_x_offsets = range(0, 2)
    tableline_y_offsets = range(0, 2)
    tableline_thickness = [1, 3]
...
class Liner(object):
    ...
    def apply(self, word_img, text_box_pnts, text_spilt, word_color):
        """
        ...
        if len(line_p) == 0:
            return word_img, text_box_pnts, text_spilt

        line_effect_func = np.random.choice(funcs, p=line_p)

        return line_effect_func(word_img, text_box_pnts, text_spilt, word_color)

    def apply_under_line(self, word_img, text_box_pnts, text_spilt, word_color):
        ...
        return dst, text_box_pnts, text_spilt

    def apply_table_line(self, word_img, text_box_pnts, text_spilt, word_color):
        """
        ...
        if is_left():
            ...
            text_spilt = [x - left_x_offset for x in text_spilt]

        if is_right():
            text_box_pnts[1][0] += right_x_offset
            text_box_pnts[2][0] += right_x_offset
            text_spilt = [x + right_x_offset for x in text_spilt]
    ...
    def apply_middle_line(self, word_img, text_box_pnts, text_spilt, word_color):
