...
class Renderer(object):
    ...
    def gen_img(self, img_index):
        ...
        word_img, text_box_pnts, text_spilt, word_color = self.draw_text_on_bg(word, font, bg)
        # cv2.imshow('word_img', word_img)
        # cv2.waitKey(4000)
        # cv2.destroyWindow('word_img')
        ...
        if apply(self.cfg.line):
            word_img, text_box_pnts, text_spilt = self.liner.apply(word_img, text_box_pnts, text_spilt, word_color)
        ...
        if self.debug:
            ...
            # cv2.waitKey(4000)
            # cv2.destroyWindow('word_img')
        word_img, img_pnts_transformed, text_box_pnts_transformed, split_points = \
            self.apply_perspective_transform(word_img, text_box_pnts, text_spilt,
        ...
        if self.debug:
            word_img, crop_bbox, split_lines = self.crop_img(word_img, text_box_pnts_transformed, split_points)
        else:
            word_img, crop_bbox, split_lines = self.crop_img(word_img, text_box_pnts_transformed, split_points)
        self.dmsg("After crop_img")

        if apply(self.cfg.noise):
            word_img = np.clip(word_img, 0., 255.).astype(np.uint8)
        ...
        if apply(self.cfg.fat):
            word_img, split_lines = self.apply_fat(word_img, split_lines)
            self.dmsg("After fat")
        word_img_2 = word_img.copy()
        for split_line in split_lines:
            cv2.line(word_img_2, (split_line, 0), (split_line, word_img.shape[0]),(255, 0, 0),thickness=1)
        return word_img, word, split_lines
    ...
    def crop_img(self, img, text_box_pnts_transformed, split_points):
        """
        ...
        for split_point in split_points:
            sp_line = self.int_around(np.around(split_point[0][0]))
            sp_line = self.int_around(np.around((sp_line - dst_bbox[0]) * (width_2 / width_1)))
            sp_lines.append(sp_line)
        return dst, dst_bbox, sp_lines
    ...
    def draw_text_on_bg(self, word, font, bg):
        ...
        if apply(self.cfg.random_space):
            ...
            text_x, text_y, word_width, word_height, text_spilt = self.draw_text_with_random_space(pil_img, draw, font,
                                                                                                   word, word_color,
                                                                                                   bg_width, bg_height)
            self.dmsg("After random_space transform")
            ...
            if apply(self.cfg.seamless_clone):
                np_img, text_spilt = self.draw_text_seamless(font, bg, word, word_color, word_height, word_width,
                                                             chars_size, width, offset)
                self.dmsg("After seamless_clone transform")
            else:
                c_x = text_x - offset[0]
                for i, c in enumerate(word):
                    text_spilt.append(c_x)
                    c_x += chars_size[i][0]
                self.draw_text_wrapper(draw, word, text_x - offset[0], text_y - offset[1], font, word_color)
                self.dmsg("After normal transform")
        ...
        return np_img, text_box_pnts, text_spilt, word_color

    def image_similar_scan(self):
        return None

    def draw_text_seamless(self, font, bg, word, word_color, word_height, word_width, chars_size, width, offset):
        ...
        c_x = int((bg.shape[1] - width) / 2)
        for i, c in enumerate(word):
            text_spilt.append(c_x)
            # cv2.line(np_img, (c_x, 0), (c_x, np_img.shape[0]), (0, 0, 0), thickness=1)
            c_x += chars_size[i][0]
        # cv2.imshow('dst', np_img)
        # cv2.waitKey()
        # cv2.destroyWindow('dst')

        return np_img, text_spilt

    def draw_text_with_random_space(self, pil_img, draw, font, word, word_color, bg_width, bg_height):
        ...
        for i, c in enumerate(word):
            ...
            # draw.line((c_x, 0, c_x, bg_height), fill='red')
            # pil_img.show()
            # time.sleep(4)
            text_spilt.append(c_x)
            c_x += (chars_size[i][0] + char_space_width)
        return text_x, text_y, width, height, text_spilt
    ...
    def gen_bg_from_image(self, width, height):
        ...
        """
        assert width > height
        if random.random() < self.cfg.img_bg.fraction:
        ...
        # img_bg = bg.copy()
        # for i in range(int(np.ceil(scale)) - 1):
        #     img_bg = cv2.hconcat(img_bg, bg)
        # out = img_bg[:width, :height]
    ...
    def apply_perspective_transform(self, img, text_box_pnts, text_spilt, max_x, max_y, max_z, gpu=False):
        """
        ...
        text_spilt_img = [[[x, text_box_pnts[0][1]], [x + 1, text_box_pnts[0][1]], [x + 1, text_box_pnts[2][1]],
                           [x, text_box_pnts[2][1]]] for x in text_spilt]
        dst_img, M33, dst_img_pnts = transformer.transform_image(img, gpu)
        dst_text_pnts = transformer.transform_pnts(text_box_pnts, M33)
        split_points = []
        for split in text_spilt_img:
            split_point = transformer.transform_pnts(split, M33)
            split_points.append(split_point)

        return dst_img, dst_img_pnts, dst_text_pnts, split_points
    ...
    def apply_fat(self, word_img, split_lines):
        ratio = np.random.randint(120, 200) / 100.0
        split_lines = [self.int_around(np.around(split_line * ratio)) for split_line in split_lines]
        word_img = cv2.resize(word_img, None, fx=ratio, fy=1.0, interpolation=cv2.INTER_LINEAR)
        return word_img, split_lines
